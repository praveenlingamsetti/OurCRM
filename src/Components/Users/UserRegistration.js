...
const UserRegister = () => {
  ...
  const changeHandler = (e) => {
    ...
    if (name === "statusValue") {
      // Update user role, and handle SalesPerson specific logic
      ...
      if (value === "SalesPerson") {
        ...
        // Filter out users who are not SalesPerson
        setReportingToUsers(
          extUsers.filter((item) => {
            return item.authorities.some(
              (auth) => auth.authority !== "SalesPerson"
            );
          })
        );
      }
  ...
  // Function to update salesperson details in state
  const salesPersonHandler = (e) => {
    const { name, value } = e.target;
    setSalesPerson({ ...SalesPerson, [name]: value });
  };

  // Function to update reportingTo state
  ...
  const submitHandler = async (e) => {
    try {
      ...
      // If user is a SalesPerson, also submit salesperson details
      if (isSalesPerson) {
  ...
  // Fetch all users on component mount and store them in state
  useEffect(() => {
    try {
      const initialFetch = () => {
        api
  ...
  // Check for user authentication token and navigate to login if not present
  useEffect(() => {
                      ...
                      <div className="col-md-6 mt-3">
                        ...
                        <div className="form-group mt-3">
                          ...
                          <input
                            ...
                            className="form-control"
                            pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
                            title="Must contain at least one number, one uppercase and one lowercase and at least 8 or more characters"
                            required
                          />
                          <span
                            type="button"
                        ...
                        <>
                          <div className="col-md-6">
                            <div className="form-group mt-3">
                              ...
                              <input
                                type="text"
                                value={SalesPerson.frequency}
                                onChange={salesPersonHandler}
                                name="frequency"
                                id="frequency"
                                className="form-control"
                                required
                          ...
                          <div className="col-md-6">
                            <div className="form-group mt-3">
                              <label htmlFor="maxTarget">
                                Max Target <span className="required">*</span>
                              </label>
                              <input
                                type="text"
                                value={SalesPerson.maxTarget}
                                onChange={salesPersonHandler}
                                name="maxTarget"
                                id="maxTarget"
                                className="form-control"
